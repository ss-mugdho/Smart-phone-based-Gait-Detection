...
lebel = {};
for i = 1:N
    x = X(1+(i-1)*150:150+(i-1)*150);
    y = Y(1+(i-1)*150:150+(i-1)*150);
    z = Z(1+(i-1)*150:150+(i-1)*150);
    x1 = X1(1+(i-1)*150:150+(i-1)*150);
    y1 = Y1(1+(i-1)*150:150+(i-1)*150);
    z1 = Z1(1+(i-1)*150:150+(i-1)*150);
    n = length(x);
    a0 = [x y z]';
    g0 = [x1 y1 z1]';
    %plot(a0(3,:))
    ...
    meanval = [meana meang];
    %type = 'abnormal';
    features = [meanval rms maxval maxcorr activity mobility complexity];
    T = table(features);
